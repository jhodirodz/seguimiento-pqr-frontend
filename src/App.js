...
2.  Una "Categoria del reclamo" que sea específica y descriptiva (ej. "Solicitud de documentos contrato", "Error en facturación servicio internet", "Falla técnica línea telefónica", "Problema calidad señal TV"). Evita categorías de una sola palabra genérica como "Contrato" o "Facturación" si se puede ser más específico.
    ...
    const responseSchema = { type: "OBJECT", properties: { "analisis_ia": { "type": "STRING" }, "categoria_reclamo": { "type": "STRING" } }};
...
const getAIPriority = async (obsText) => {
    const prompt = `Asigna "Prioridad" ("Alta", "Media", "Baja") a obs: ${obsText || 'N/A'}. Default "Media".`;
    const responseSchema = { type: "OBJECT", properties: { "prioridad": { "type": "STRING" } }};
    try {
        const responseText = await callMyBackend(prompt, responseSchema);
        return JSON.parse(responseText).prioridad || 'Media';
...
const getAISummary = async (caseData) => {
    ...
    Niveles: ${[1,2,3,4,5].map(n => caseData[`Nivel_${n}`] || 'N/A').join(', ')}, Operación: ${caseData.Tipo_Operacion || 'N/A'}`;
     const responseSchema = { type: "OBJECT", properties: { "resumen_cliente": { "type": "STRING" } }};
    try {
        const responseText = await callMyBackend(prompt, responseSchema);
        return JSON.parse(responseText).resumen_cliente || 'No se pudo generar resumen.';
...
Tipo de Contrato: ${contractType || 'No especificado'}`;
    ...
    const responseSchema = { type: "OBJECT", properties: { "proyeccion_respuesta_ia": { "type": "STRING" } }};
    try {
        const responseText = await callMyBackend(prompt, responseSchema);
        return JSON.parse(responseText).proyeccion_respuesta_ia || 'No se pudo generar proyección.';
...
Responde SOLO con JSON: {"area": "...", "motivo": "..."}`;
    ...
    const responseSchema = { type: "OBJECT", properties: { "area": { "type": "STRING" }, "motivo": { "type": "STRING" } }};
    try {
        const responseText = await callMyBackend(prompt, responseSchema);
...
const handleScanFileUploadBackend = async (file, caseToScan, db, userId, displayModalMessage, updateCaseInFirestore) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async () => {
            ...
            try {
                const responseText = await callMyBackend(prompt, null, base64ImageData, file.type);
...
function App() {
    ...
    const handleScanFileUpload = async (event) => {
        ...
        try {
             await handleScanFileUploadBackend(file, caseToScan, db, userId, displayModalMessage, updateCaseInFirestore);
        } catch (error) {
             // Error is already displayed by the backend handler
        } finally {
            setIsScanning(false);
            setCaseToScan(null);
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-5 mb-4 border-l-2 border-green-300">
                                        ...
                                        {selectedCase.requiereDevolucionDinero && (
                                            <div className="contents"> 
                                                ...
                                                <div><label htmlFor="fechaEfectivaDevolucion" className="block text-sm font-medium text-gray-700 mb-1">Fecha Efectiva Devolución:</label><input type="date" id="fechaEfectivaDevolucion" name="fechaEfectivaDevolucion" value={selectedCase.fechaEfectivaDevolucion || ''} onChange={(e) => handleModalFieldChange('fechaEfectivaDevolucion', e.target.value)} className="block w-full input-form" disabled={!selectedCase.requiereAjuste || !selectedCase.requiereDevolucionDinero}/></div>
                                            </div>
                                        )}
                                    </div>
                                )}
